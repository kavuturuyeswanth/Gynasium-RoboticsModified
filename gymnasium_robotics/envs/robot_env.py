...
class BaseRobotEnv(GoalEnv):
    ...
    def step(self, action, goal):
        ...
        info = {
            "is_success": self._is_success(grip_pos, goal),
        }

        terminated = self.compute_terminated(grip_pos, goal, info)
        truncated = self.compute_truncated(grip_pos, goal, info)

        reward = self.compute_reward(grip_pos, goal, info)
